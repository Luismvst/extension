...
class OnTimeAdapter(CarrierAdapter):
    ...
    async def _create_shipments_bulk_mock(self, orders: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        logger.info(f"OnTime shipments", duration_ms=len(orders) * 150
        )
    ...
    async def _get_shipment_label_real(self, shipment_id: str) -> bytes:
        ...
        try:
            ...
            self.logger.error(f"Timeout getting label from OnTime: {e}", exc_info=True)
            raise Exception(f"Request timeout: {e}")
        except httpx.HTTPError as e:
            self.logger.error(f"HTTP error getting label from OnTime: {e}", exc_info=True)
            raise Exception(f"HTTP error: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error getting label from OnTime: {e}", exc_info=True)
    ...
    async def _cancel_shipment_real(self, shipment_id: str, reason: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Timeout cancelling shipment in OnTime: {e}", exc_info=True)
            raise Exception(f"Request timeout: {e}")
        except httpx.HTTPError as e:
            self.logger.error(f"HTTP error cancelling shipment in OnTime: {e}", exc_info=True)
            raise Exception(f"HTTP error: {e}")
        except Exception as e:
            self.logger.error(f"Unexpected error cancelling shipment in OnTime: {e}", exc_info=True)
            raise Exception(f"Request failed: {e}")
