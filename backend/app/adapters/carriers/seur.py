...
This module implements the SeurAdapter for interacting with
SEUR carrier APIs for shipment creation and tracking.
...
class SeurAdapter(CarrierAdapter):
    """SEUR carrier adapter implementation."""
    
    def __init__(self):
        """Initialize SEUR adapter."""
        self.api_key = settings.seur_api_key
        self.base_url = settings.seur_base_url
        self.mock_mode = settings.seur_mock_mode
    ...
    async def _create_shipment_mock(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        shipment_id = f"SEUR-{order_id[-8:].upper()}{hash(order_id) % 10000:04d}"
        tracking_number = f"SE{order_id[-8:].upper()}{hash(order_id) % 10000:04d}"
        
        result = {
            "shipment_id": shipment_id,
            "expedition_id": shipment_id,  # SEUR uses shipment_id as expedition_id
            "tracking_number": tracking_number,
            "status": "CREATED",
            "label_url": f"https://mock.seur.com/labels/{shipment_id}",
            "estimated_delivery": (datetime.utcnow() + timedelta(days=3)).isoformat(),
            "cost": 18.75 + (weight * 2.5),
            "currency": "EUR",
            "carrier": "SEUR",
            "service": "STANDARD",
    ...
    async def _create_shipment_real(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Transform order data to SEUR format
    ...
    def _transform_order_to_shipment(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        """Transform order data to SEUR shipment format."""
        return {
    ...
    async def _get_cached_shipment(self, idempotency_key: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "tracking_number": f"SE{idempotency_key[:8]}",
            "status": "CREATED",
            "cached": True
        }
    ...
    async def _get_shipment_status_mock(self, expedition_id: str) -> Dict[str, Any]:
        ...
        result = {
            ...
            "status": status,
            "tracking_number": f"SE{expedition_id[-8:]}",
            "label_url": f"https://mock.seur.com/labels/{expedition_id}" if status in ["LABELED", "IN_TRANSIT", "OUT_FOR_DELIVERY", "DELIVERED"] else None,
    ...
    def process_webhook_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process webhook event from SEUR."""
